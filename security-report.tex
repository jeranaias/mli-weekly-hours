...
\noindent The data flows through six stages, all within the browser except the AI call:

\begin{center}
\begin{tikzpicture}[
  sbox/.style={draw, rounded corners=4pt, minimum width=2.6cm, minimum height=0.85cm, align=center, font=\footnotesize\bfseries},
  arr/.style={-{Stealth[length=3mm]}, thick},
]
  % Vertical flow — no overlap possible
  \node[sbox, fill=milgreen!15, draw=milgreen] (excel) at (0, 0) {1. Excel File (with PII)};
  \node[sbox, fill=infoblue, draw=blue!50!black] (sanitize) at (0, -1.5) {2. Sanitizer --- strip columns};
  \node[sbox, fill=infoblue, draw=blue!50!black] (tokenize) at (0, -3) {3. Tokenizer --- names $\rightarrow$ MLI\_1, MLI\_2};
  \node[sbox, fill=warnbox, draw=orange!70!black] (gemini) at (0, -4.5) {4. Google Gemini AI};
  \node[sbox, fill=infoblue, draw=blue!50!black] (detokenize) at (0, -6) {5. Detokenizer --- tokens $\rightarrow$ names};
  \node[sbox, fill=safebox, draw=safegreen] (output) at (0, -7.5) {6. Chat Display \& PDF Export};

  % Arrows with side labels
  \draw[arr] (excel) -- node[right, font=\scriptsize, xshift=4pt] {\color{stripred}Student names, locations, notes removed} (sanitize);
  \draw[arr] (sanitize) -- node[right, font=\scriptsize, xshift=4pt] {\color{tokenblue}Real names replaced with anonymous tokens} (tokenize);
  \draw[arr] (tokenize) -- node[right, font=\scriptsize, xshift=4pt] {\color{tokenblue}Only tokens + hours data sent externally} (gemini);
  \draw[arr] (gemini) -- node[right, font=\scriptsize, xshift=4pt] {\color{tokenblue}AI responds using tokens} (detokenize);
  \draw[arr] (detokenize) -- node[right, font=\scriptsize, xshift=4pt] {\color{safegreen}Full names restored locally for display \& PDF} (output);

  % Boundary labels
  \begin{scope}[on background layer]
    \node[draw=milgreen, dashed, thick, rounded corners=8pt, fill=milgreen!3,
          fit=(excel)(sanitize)(tokenize)(detokenize)(output),
          inner sep=14pt, label={[font=\footnotesize\bfseries\color{milgreen}]left:Browser}] {};
  \end{scope}
  \begin{scope}[on background layer]
    \node[draw=orange!70!black, dashed, thick, rounded corners=8pt, fill=orange!3,
          fit=(gemini),
          inner sep=10pt, label={[font=\footnotesize\bfseries\color{orange!70!black}]left:External}] {};
  \end{scope}
...
\begin{center}
\begin{tikzpicture}[
  row/.style={font=\scriptsize\ttfamily},
]
  % Before
  \node[font=\small\bfseries] at (-3, 2) {After Stripping};
  \node[row] at (-3, 1.3) {Morgan,A3,Full Section,6,,0730};
  \node[row] at (-3, 0.8) {Smith,B2,Split Section,4,,0800};
  \node[row] at (-3, 0.3) {Morgan,A3,Spec Asst,1,,1300};

  % Arrow
  \draw[-{Stealth[length=4mm]}, ultra thick, color=tokenblue] (0.8, 1) -- node[above, font=\small\bfseries\color{tokenblue}] {Tokenize} (2.8, 1);

  % After
  \node[font=\small\bfseries] at (6.4, 2) {Sent to AI};
  \node[row, color=tokenblue] at (6.4, 1.3) {MLI\_1,A3,Full Section,6,,0730};
  \node[row, color=tokenblue] at (6.4, 0.8) {MLI\_2,B2,Split Section,4,,0800};
  \node[row, color=tokenblue] at (6.4, 0.3) {MLI\_1,A3,Spec Asst,1,,1300};

  % Note
  \node[draw=tokenblue, fill=infoblue, rounded corners=3pt, font=\scriptsize, text width=9cm, align=center] at (1.8, -0.6)
...
\begin{center}
\begin{tikzpicture}[
  phase/.style={draw, rounded corners=6pt, minimum width=2.8cm, minimum height=1.1cm, align=center, font=\small\bfseries},
  ...
  \node[create] (upload) at (0, 0) {File Upload\\{\scriptsize\normalfont Excel dropped}};
  \node[live] (process) at (4, 0) {Sanitize +\\Tokenize\\{\scriptsize\normalfont In browser RAM}};
  \node[live] (ai) at (8, 0) {AI Chat\\{\scriptsize\normalfont Tokens only}};
  \node[live] (result) at (8, -2.4) {Form Filled\\{\scriptsize\normalfont Local state}};
  \node[live] (pdf) at (4, -2.4) {PDF Export\\{\scriptsize\normalfont Local download}};
  \node[destroy] (gone) at (0, -2.4) {Page Close\\{\scriptsize\normalfont All data gone}};
